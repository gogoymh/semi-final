...
from additive_decomp import Unet as net
from additive_decomp import sparse_one, sparse_two, is_labeled, is_unlabeled, get_norm_sparse1, get_norm_sparse2, num_parameter, get_norm_dense
...
for epoch in range(300):
    ...
    for x_U, _ in unlabeled_loader:
        ...
        ## ---- L2 Norm for Sparsity ---- ##
        L2_norm = (get_norm_sparse1(model) + get_norm_sparse2(model))/2
        L2_norm /= num_param # averaging with the number of parameters
        
        ## ---- Total ---- ##
        loss = lambda_1 * loss_U #+ lambda_3 * L2_norm
        loss.backward()
        optimizer.step()
    ...
    print("[Epoch:%d] [Labeled:%f] [L1:%f] [Unlabeled:%f] [L1:%f]" % ((epoch+1), running_loss1, running_loss3, running_loss2, running_loss4))
