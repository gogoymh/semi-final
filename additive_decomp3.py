...
class random_uniform(nn.Module):
    def __init__(self, value, prob=0.5):
        super().__init__()
    ...
    def forward(self, x):
        if self.training:
            #prob = torch.rand(x.shape).to(x.device)#self.value
            #prob = torch.where(torch.rand(x.shape) < self.prob, self.value*torch.ones(x.shape), torch.ones(x.shape)).to(x.device)
            prob = torch.where(torch.rand(x.shape) < self.prob, torch.normal(1, 1, x.shape), torch.ones(x.shape)).to(x.device)
            x = prob * x
...
class additive_decomposed_weight(new_Module):
    def __init__(self, shape, value=None, scale=1):
        ...
        if value is None: # for weight
            ...
            self.sparse_weight1 = nn.Parameter(scale*torch.randn(shape))
            self.sparse_weight2 = nn.Parameter(scale*torch.randn(shape)) 
            
        else: # for bias or initialization of Norm
            self.dense_weight = nn.Parameter(0.5*value*torch.ones(shape))
            self.sparse_weight1 = nn.Parameter(0.5*value*torch.ones(shape))
            self.sparse_weight2 = nn.Parameter(value*torch.zeros(shape)) 
        
        self.dropout1 = random_uniform(0.1,0.05)#nn.Dropout(0.05)##
        self.dropout2 = random_uniform(0.1,0.2)#nn.Dropout(0.2)##
        
    def forward(self):
        if self.sparse1:
            sparse_weight = self.dropout1(self.sparse_weight1)
        else:
            sparse_weight = self.dropout2(self.sparse_weight1)
