...
for epoch in range(300):
    ...
    for x_U, _ in unlabeled_loader:
        ...
        L1_norm = (get_norm_sparse1(model) + get_norm_sparse2(model))/2
        #L1_norm /= num_param # averaging with the number of parameters
        
        ## ---- L2 norm for Density ---- ##
        L2_norm = get_norm_dense(model)
        #L2_norm /= num_param
        
        ## ---- Total ---- ##
        loss = loss_L + lambda_1 * loss_U + lambda_2 * L1_norm #+ lambda_3 * L2_norm
        loss.backward()
        optimizer.step()
    ...
    print("[Epoch:%d] [Loss:%f] [Labeled:%f] [Unlabeled:%f] [L1:%f] [L1:%f]" % ((epoch+1), running_loss, running_loss1, running_loss2, running_loss3, running_loss4))
