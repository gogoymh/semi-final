...
class additive_decomposed_weight(new_Module):
    def __init__(self, shape, value=None, scale=1):
        ...
        if value is None: # for weight
            self.base = nn.ParameterList()
            for i in range(4):
                self.base.append(nn.Parameter(scale*torch.randn(shape)))
            ...
            self.base = nn.ParameterList()
            for i in range(4):
                self.base.append(nn.Parameter(value*scale*torch.ones(shape)))
            
            self.ultrasonograpy = nn.Parameter(torch.ones(shape))
    ...
    def forward(self):
        ...
        #weight = self.ultrasonograpy * self.sigmoid(device_weight + part_weight)
        #weight = self.base * self.sigmoid(self.ultrasonograpy[self.base_task] + device_weight * part_weight)
        #weight = self.base[self.base_task] + self.ultrasonograpy + device_weight + part_weight
        
        
        weight = self.ultrasonograpy * self.sigmoid(self.base[self.base_task] * device_weight * part_weight)
